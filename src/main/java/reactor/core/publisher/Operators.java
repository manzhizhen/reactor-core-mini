   ...
   */
	...
	public static class MonoSubscriber<I, O>
		...
		public Object scanUnsafe(Scannable.Attr key) {
			if (key == Scannable.Attr.CANCELLED) return isCancelled();
			if (key == Scannable.Attr.TERMINATED) return state == HAS_REQUEST_HAS_VALUE || state == NO_REQUEST_HAS_VALUE;
			if (key == Scannable.Attr.PREFETCH) return Integer.MAX_VALUE;
	...
	static abstract class BaseFluxToMonoOperator<I, O> implements InnerOperator<I, O>,
		...
		public Object scanUnsafe(Scannable.Attr key) {
			...
			if (key == Scannable.Attr.RUN_STYLE) return RunStyle.SYNC;
	...
	static final class ScalarSubscription<T>
		...
		@Nullable
		public Object scanUnsafe(Scannable.Attr key) {
			if (key == Scannable.Attr.TERMINATED) return once == 1;
			if (key == Scannable.Attr.CANCELLED) return once == 2;
			if (key == Scannable.Attr.RUN_STYLE) return RunStyle.SYNC;
	...
	/*package*/ static class MonoInnerProducerBase<O>
		...
		public Object scanUnsafe(Scannable.Attr key) {
			if (key == Scannable.Attr.CANCELLED) return isCancelled();
			if (key == Scannable.Attr.TERMINATED) return hasCompleted(state);
			if (key == Scannable.Attr.PREFETCH) return Integer.MAX_VALUE;
			return InnerProducer.super.scanUnsafe(key);
		}
